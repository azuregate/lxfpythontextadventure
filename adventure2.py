...
room1 = Room(1, "Main Chamber", "You're standing in a dark cavern. You hear the sound of dripping water.", [None, None, None, None], None, None)
...
room3 = Room(3, "South Chamber", "You see an old chest.", [None, None, None, None], None, None)
...
room2 = Room(2, "North Chamber", "You see the SKELETON of a dwarf holding an AXE.", [None, None, None, None], None, None)
...
room4 = Room(4, "East Chamber", "You see entrance to the cave where you arrived earlier. The gate is locked. It's stained red with rust.",[None,None,None,None], None, None)

grille = Item("Grille", "The iron grille has been prised open a few inches but you can't fit through.", None, False, None, None, None, None, None, None, False)
room5 = Room(5, "West Chamber", "You see an iron grille set into the wall. Someone has prised it open.",[None,None,None,None], None, None)
room5.items = [grille]

room6 = Room(6, "Cave Entrance", "You step out blinking into the sunlight.",[None,None,None,room4], None, None)
...
player = Player(player_name, room1, [redkey], [bread])
...
def trytotake(item):
    ...
    for room_item in current_room.items:
        if room_item.name.lower() == item.lower():
            if room_item in player.inventory:
                print("You already have the", room_item.name + ".")
                return
...
def trytouse(item):
    ...
    for inventory_item in player.inventory:
        if inventory_item.name.lower() == item.lower():
            if inventory_item.usedin in [current_room.number, 0]:
