...
class Weapon(Item):
    def __init__(self, name, itemdesc, updroomdesc, portable, revealsitem, usedin, usedesc, removesroomitem, addsroomitem, useroomdesc, disposable, damage, sound_path):
        super().__init__(name, itemdesc, updroomdesc, portable, revealsitem, usedin, usedesc, removesroomitem, addsroomitem, useroomdesc, disposable)
        self.damage = damage
...
mace = Weapon("Mace", "A stubby but lethal-looking mace.", None, True, None, 0, "You swing the mace around clumsily.", None, None, None, False, 1, "/home/nate/mu_code/mace.wav")
...
def play_player_weapon_sound():
    sound_file_path = player.weapon.sound_path
    playerweapon_sound = pygame.mixer.Sound(sound_file_path)
    playerweapon_sound.play()
    return

def play_enemy_weapon_sound():

        for room_enemy in player.currentroom.enemies:

            if room_enemy.name.lower() == enemy_name.lower():
                enemy = room_enemy
                break

        sound_file_path = enemy.weapon.sound_path
        enemyweapon_sound = pygame.mixer.Sound(sound_file_path)
        enemyweapon_sound.play()
        return
...
def listinventory():
    # Print player information in a colored section
    print(Fore.YELLOW + player.name)
    ...
    # Print the inventory items in a colored section
